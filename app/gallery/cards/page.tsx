...
export default function CardsPage() {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <ComponentCard
          ...
          renderComponent={(config) => (
            <div
              className={`relative rounded-xl border p-5 bg-white dark:bg-zinc-950 overflow-hidden shadow-sm flex flex-col justify-between h-[210px] ${
                config.glowEffect
                  ? "border-red-500/50 shadow-red-500/5 dark:shadow-red-500/10"
                  : "border-slate-200 dark:border-zinc-800"
              }`}
            >
              {config.glowEffect && (
                <div className="absolute -top-10 -right-10 w-24 h-24 rounded-full bg-red-500/10 blur-xl pointer-events-none" />
              )}
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-zinc-400 mt-1 leading-relaxed line-clamp-3">
                  {config.description ||
                    "Protect assets globally with automatic threat shields and low latency access tunnels."}
                </p>
              </div>
              <button className="inline-flex items-center gap-1.5 text-xs font-bold text-red-500 hover:text-red-400 transition-colors mt-4 self-start cursor-pointer">
  ...
  return (
    <div className="relative rounded-xl border border-red-500/50 p-6 bg-white dark:bg-zinc-950 shadow-md">
      <h4 className="text-lg font-bold text-foreground">${config.cardTitle}</h4>
      <p className="text-sm text-slate-500 dark:text-zinc-400 mt-2">${config.description}</p>
      <button className="inline-flex items-center gap-2 text-sm font-bold text-red-500 hover:underline mt-4">
        ...
        <ComponentCard
          ...
          renderComponent={(config) => (
            <div
              className={`rounded-xl border p-5 bg-white dark:bg-zinc-950 flex flex-col justify-between h-[210px] select-none ${
                config.featured
                  ? "border-red-500 ring-2 ring-red-500/20 shadow-md shadow-red-950/20"
            ...
            >
              <div>
                <div className="flex items-center justify-between">
                  ...
                  {config.featured && (
                    <span className="px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase bg-red-100 text-red-800 dark:bg-red-950/50 dark:text-red-400 border border-red-500/20">
                      Popular
                    </span>
                  )}
                </div>
                ...
                <ul className="space-y-1.5 mt-4 text-[10px] text-slate-500 dark:text-zinc-400">
